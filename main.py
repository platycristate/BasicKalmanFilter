...
command_var = 0.0
mean_acc = 0
mean_init = np.array([0, 0.2]).reshape(2, 1)
...
if __name__ == '__main__':
    ...
    accelarations = [np.random.normal(0.0, 0) for i in range(N)]
    for i, acc in zip(range(N), accelarations):
        ...
        pred_covs.append(kf.cov[0, 0])
        pred_covs_v.append(kf.cov[1, 1])
    ...
    axs[0].plot([mean + 1 * np.sqrt(cov) for mean, cov in zip(pred_positions, pred_covs)], '--')
    axs[0].plot([mean - 1 * np.sqrt(cov) for mean, cov in zip(pred_positions, pred_covs)], '--')
    ...
    axs[1].plot([mean + 1 * np.sqrt(cov) for mean, cov in zip(pred_velocities, pred_covs_v)], '--')
    axs[1].plot([mean - 1 * np.sqrt(cov) for mean, cov in zip(pred_velocities, pred_covs_v)], '--')
